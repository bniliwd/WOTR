    ...
    public class Main
    {
        ...
        static class BlueprintsCache_Patch
        {
            ...
            public static void Postfix()
            {
                ...
                ModifyEnemyEquipment();
        
            }
        
            public static void ModifyEnemyEquipment()
            {
                PFLog.Mods.Log("加载Boss掉落补丁开始...");
        
                //单位id，装备id(可以考虑写明物品位置)...
                ...
                foreach (List<string> combinations in UNITS)
                {
                    ...
                    if (body == null) {
                        PFLog.Mods.Error(string.Format("未获取到角色{0}的Body属性", combinations[0]));
                        continue;
                    }
                    ...
                    foreach (string id in combinations.Skip(1))
                    {
                        ...
                        if (unknown == null) {
                            PFLog.Mods.Error(string.Format("未获取到装备{0}蓝图", id));
                            continue;
                        }
                        switch (unknown)
                        {
                            ...
                            case BlueprintItemEquipmentHand m_PrimaryHand://主手
                                unit.Body.m_PrimaryHand = m_PrimaryHand.ToReference<BlueprintItemEquipmentHandReference>();
                                break;
                            ...
                            case BlueprintItemEquipmentNeck m_Neck:
                                ...
                                break;
                            case BlueprintItemEquipmentRing m_Ring:
                                //临时方案
                                if (combinations[0].StartsWith("91cd4")) { 
                                    body.m_Ring2 = m_Ring.ToReference<BlueprintItemEquipmentRingReference>(); 
                                } else { 
                                    body.m_Ring1 = m_Ring.ToReference<BlueprintItemEquipmentRingReference>(); 
                                }
                                break;
        ...
        static class WeaponFocusParametrized_Patch
        {
            ...
            {
                FeatureParam baseParam = __instance.Param;
                UnitFact baseFact = __instance.Fact;
